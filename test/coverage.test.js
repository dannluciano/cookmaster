...
beforeAll(async () => {
	try {
		...
		throw new Error(
			`Não foi possível fazer a leitura da cobertura\n${error.message}`,
		);
	}
});
...
describe.each([
	...
	[13, 60, 100],
	// [14,90,150]
])("%p - Crie testes de integração que cubram no mínimo %p porcento dos arquivos em src com um mínimo de %p linhas cobertas", (_testId, percentage, coveredLines) => {
